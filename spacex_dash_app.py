...
def get_scatter_chart(launch_site, payload_range):
    if launch_site == 'All':
        ...
        fig = px.scatter(filtered_df, x='Payload Mass (kg)', y='class', color='Booster Version Category', title='Site {} - payload mass between {:8,d}kg and {:8,d}kg'.format(launch_site,int(payload_range[0]),int(payload_range[1])))
